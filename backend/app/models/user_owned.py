...
class Character(IdMixin, TimestampMixin, ActiveMixin, OwnedModelMixin, Base):
    ...
    origin_story_id: Mapped[int | None] = mapped_column(Integer, index=True)
...
class Loadout(IdMixin, TimestampMixin, ActiveMixin, OwnedModelMixin, Base):
    ...
    notes: Mapped[str | None] = mapped_column(Text)
    role_id: Mapped[int | None] = mapped_column(Integer, index=True)
    spec_id: Mapped[int | None] = mapped_column(Integer, index=True)
...
class CharacterCrewSkillRelation(IdMixin, TimestampMixin, ActiveMixin, OwnedModelMixin, Base):
    ...
    crew_skill_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
...
class OperationLockout(IdMixin, TimestampMixin, ActiveMixin, OwnedModelMixin, Base):
    ...
    character_id: Mapped[int] = mapped_column(ForeignKey("characters.id"), nullable=False, index=True)
    boss_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    operation_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    difficulty_id: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
